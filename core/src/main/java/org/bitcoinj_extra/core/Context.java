 ...
 * bitcoinj_extra for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating bitcoinj_extra {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use bitcoinj_extra (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a bitcoinj_extra context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using bitcoinj_extra!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing bitcoinj_extra via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.bitcoinj_extra.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.bitcoinj_extra.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.bitcoinj_extra.core.NetworkParameters} specified when this context was (auto) created. The

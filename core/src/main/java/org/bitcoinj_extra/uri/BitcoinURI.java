...
package org.bitcoinj_extra.uri;

import org.bitcoinj_extra.core.*;
import org.bitcoinj_extra.params.AbstractBitcoinNetParams;
...
public class BitcoinURI {
    ...
    /**
     * URI for Bitcoin network. Use {@link org.bitcoinj_extra.params.AbstractBitcoinNetParams#BITCOIN_SCHEME} if you specifically
     * need Bitcoin, or use {@link org.bitcoinj_extra.core.NetworkParameters#getUriScheme} to get the scheme
     ...
     *
     * @param details The CoinDetails that determine which network the URI is from, or null if you don't
     *               have any expectation about what network the URI is for and wish to check yourself.
     ...
     */
    public BitcoinURI(@Nullable CoinDetails details, String input) throws BitcoinURIParseException {
        checkNotNull(input);
        ...
        if (details == null) {
            ...
            if (colonIndex == -1)
                throw new BitcoinURIParseException("No colon in URI");
            scheme = input.substring(0, colonIndex);
            details = Networks.get(scheme);
            if (details == null)
                throw new BitcoinURIParseException("Could not find network for uri.");
        } else
            scheme = details.getUriScheme();

        // Allow nu
        if (scheme.equals("Nu") && input.startsWith("nu"))
            scheme = "nu";
        ...
        if (addressSplitTokens.length == 0)
            throw new BitcoinURIParseException("No data found after the " + scheme + ": prefix");
        ...
        parseParameters(details, addressToken, nameValuePairTokens);

        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a Bitcoin address for this network
            try {
                Address address = Address.fromBase58(details, addressToken);
    ...
    /**
     * @param details             The CoinDetails or null
     * @param nameValuePairTokens The tokens representing the name value pairs (assumed to be
     *                            separated by '=' e.g. 'amount=0.2')
     */
    private void parseParameters(@Nullable CoinDetails details, String addressToken, String[] nameValuePairTokens) throws BitcoinURIParseException {
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (FIELD_AMOUNT.equals(nameToken)) {
                ...
                try {
                    Monetary amount = details.parseCoin(valueToken);
                    if (!details.isShapeShift() && ((Coin) amount).isGreaterThan(((NetworkParameters) details).getMaxMoney()))
                ...
                if (nameToken.startsWith("req-")) {
                    ...
                    try {
                        if (valueToken.length() > 0 && (!details.isShapeShift() || !nameToken.equals(FIELD_PAYMENT_REQUEST_URL)))
    ...
    public static String convertToBitcoinURI(Address address, Coin amount,
        ...
        return convertToBitcoinURI(address.getSelectedCoinDetails(), address.toString(), amount, label, message);
    }

    /**
     * Simple Bitcoin URI builder using known good fields.
     *
     * @param details The coin details that determine which network the URI
     *                is for.
     * @param address The Bitcoin address
     * @param amount  The amount
     * @param label   A label
     ...
     * @return A String containing the Bitcoin URI
     */
    public static String convertToBitcoinURI(CoinDetails details,
        ...
        String scheme = details.getUriScheme();
